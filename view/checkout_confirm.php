<?php
if(isset($loadOneBill) && (is_array($loadOneBill))) {
    extract($loadOneBill);
}
 if(isset($loadAllPro) && (count($loadAllPro) >0)){
    extract($loadAllPro);
    $html_AllBllPro='';
foreach($loadAllPro as $item){
    extract($item);
    $html_AllBllPro.='
    <tr>
...
?>
<main>
    <a href="index.php?page=mybill">Xem đơn hàng của bạn tại đây</a>
    <section class="">
        ...
        <div class="cart-box padding-all-2rem">
            ...
            <h1 class="mt-6 text-center">Sản phẩm</h1>
            <div class="w-full mt-6 text-center">
                <table class="w-full" border=1>
                    <tr>
                        <th>Ảnh</th>    
